...
namespace WebApp.DAL.Repositories
{
    public class OrderRepository(UnitOfWork unitOfWork) : IOrderRepository
    {
        ...
        public async Task<int> UpdateStatuses(long[] orderIds, string newStatus, CancellationToken token)
        {
            const string sql = @"
            update orders
            set 
            ...
            var affected = await conn.ExecuteAsync(new CommandDefinition(
                sql, new { OrderIds = orderIds, NewStatus = newStatus }, cancellationToken: token));
